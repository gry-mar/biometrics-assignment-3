...
import src.anti_spoof.third_party
sys.path.append('../../')

import timm
import src.anti_spoof.clip as clip
from src.anti_spoof.prompt_templates import spoof_templates, real_templates
...
class flip_it(nn.Module):
  ...
  def __init__(self):
    ...
    self.model, _ = clip.load("ViT-B/16", 'cpu')
  ...
  def forward(self, input, norm_flag=True):
    ...
    real_texts = clip.tokenize(real_templates)
